...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    response = client.chat.completions.create(
        model="gemini-2.5-flash", # Updated to a Gemini model
    ...
    # Append the filename to the host URL
    full_log_url = f"{PUBLIC_HOST_URL.rstrip('/')}/{LOG_FILE}" if PUBLIC_HOST_URL else LOG_FILE
    parsed["log_url"] = full_log_url
